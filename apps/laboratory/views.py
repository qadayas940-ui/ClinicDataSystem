...
def archive(request, pk):
    item = get_object_or_404(LabOrder, pk=pk)
    item.soft_delete()
    log_audit(request, "delete", "LabOrder", item.pk, str(item))
    messages.success(request, "طلب المختبر نُقل إلى سلة المحذوفات.")
    return redirect("laboratory:list")
...
def restore(request, pk):
    item = get_object_or_404(LabOrder.all_objects, pk=pk, deleted_at__isnull=False)
    item.restore()
    log_audit(request, "restore", "LabOrder", item.pk, str(item))
